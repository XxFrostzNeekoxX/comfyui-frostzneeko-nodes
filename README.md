...
- 👁️ **One-node Face Detailer** — detection + detail in a single node
...
You'll also need Ultralytics detection models (`.pt` files) in `ComfyUI/models/ultralytics/bbox/` or `ComfyUI/models/ultralytics/segm/`. Any YOLO model works — face detection, person detection, etc.
...
Automatic face/feature detection and inpainting in **one node**.
...
**Do I need other custom node packs installed?**

**No.** Everything here is self-contained. For the Face Detailer you only need the `ultralytics` pip package and YOLO model files (`.pt`).

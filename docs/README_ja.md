...
- 📄 **ファイルからプロンプト** — `.txt`からプロンプトを読み取り、ステートフルな行モードで処理し、ワイルドカード解決とLoRAインライン読み込みに対応
- ⚡ **Supreme KSampler** — 内蔵の空Latent、ライブプレビュー、アップスケーラー、ディテーラートグル、全部入り
- 👁️ **ワンノード Face Detailer** — 検出 + ディテールを1つのノードで実行
...
- Face Detailer: マスクプレビュー、tiled VAE encode/decode、refinerステージ、任意の `SIGMAS` 入力を追加。
- Image Saver: 生成情報（日時、seed、sampler、prompt、LoRA）を含む見やすいPNGメタデータを追加。
- Image Saver: その整形メタデータをON/OFFできる `save_pretty_metadata` トグルを追加。
...
ComfyUI Managerで **FrostzNeeko** を検索してインストールをクリック。
...
### 方法3: ZIPダウンロード
ダウンロードして `ComfyUI/custom_nodes/comfyui-frostzneeko-nodes/` に展開。

インストール後、**ComfyUIを再起動**。すべてのノードがノードメニューの **FrostzNeeko 🔹** に表示されます。
...
Ultralytics検出モデル（`.pt`ファイル）も `ComfyUI/models/ultralytics/bbox/` または `ComfyUI/models/ultralytics/segm/` に必要です。任意のYOLOモデルが使えます。
...
| 機能 | 説明 |
|---|---|
| **行選択** | `increment`、`decrement`、`random`、`random no repetitions`、`fixed` |
| **バッチカウンター** | 非表示の `count` をフロントエンドJSが自動入力し、新しいキューでリセット |
| **開始行** | `line_to_start_from` で開始行を指定 |
...
| **No-LoRA CLIP** | LoRAパッチなしのクリーンな `no_lora_clip` 出力 |

**出力:** `MODEL`、`CLIP`、`no_lora_clip`、`VAE`、`CONDITIONING`、`processed_prompt`、`raw_prompt`、`line_number`
...
自動顔検出＆インペインティングを**1つのノード**で実行。

**出力:** `IMAGE`、`mask_preview`
...
追加機能付きCLIPエンコーダー、クリーンなチェックポイントローダー、フォーマット＆命名を完全制御できる画像保存ノード（`save_pretty_metadata`対応）。
...
<p align="center">
  ...
  <br/>
  <sub>❤️ を込めて FrostzNeeko が作成</sub>
</p>

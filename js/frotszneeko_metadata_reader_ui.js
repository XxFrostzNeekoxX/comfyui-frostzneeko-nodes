...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData, _app) {
        ...
        nodeType.prototype.onExecuted = function (data) {
            ...
            if (!data) return;
            if (!this.widgets) return;
            const metaW = this.widgets.find((w) => w.name === "metadata_textbox");
            if (metaW && Array.isArray(data.metadata_textbox) && data.metadata_textbox.length > 0) {
                metaW.value = data.metadata_textbox[0] || "";
            }
            this.setDirtyCanvas(true, true);
        };
    },
